...
Stream<Userscreen?>get user{
  return _auth.authStateChanges().map(((User? user) =>_userfromfiebase(user) ));
...
try {
  UserCredential result = await  _auth.signInWithEmailAndPassword(email: email, password: passworde);
...
try {
  UserCredential result = await  _auth.createUserWithEmailAndPassword(email: email, password: passworde);
  User? user= result.user;
